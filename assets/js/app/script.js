...
$('#tags__ul').on('click', 'li', function () {
	var $self = $(this);
	$self.addClass('active').siblings().removeClass('active');
	tag1.hide();
	$('.' + $(this).attr('tags')).delay(50).fadeIn(350);
})
...
var CodePenEmbed = {
	...
	_getParamsFromAttributes : function (e) {
		...
		for (var r = 0, i = n.length; r < i; r++)
			name = n[r].name, name.indexOf("data-") === 0 && (t[name.replace("data-", "")] = n[r].value);
		return t
	},
	_convertOldDataAttributesToNewDataAttributes : function (e) {
		return e.href && (e["slug-hash"] = e.href),
		e.type && (e["default-tab"] = e.type),
		e.safe && (e["safe"] == "true" ? e.animations = "run" : e.animations = "stop-after-5"),
		e
	},
...
function afterPjax() {
	...
	function show_comment() {
		var $src = $('.ds-thread');
		DUOSHUO.EmbedThread($src);
		$('.disqus_thread .container').remove($src).append($src);
	};
	show_comment();
	// 
	$('#post__content .artZoom').artZoom(); 
}
afterPjax();
